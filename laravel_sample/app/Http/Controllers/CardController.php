...
class CardController extends Controller
{
    ...
    public function cardAuthorize(Request $request)
    {
        ...
        if ($response_data instanceof CardAuthorizeResponseDto) {
            ...
            return redirect()->action('CardController@authorizeResult', ['orderId' => $request->request->get("orderId"),'data' => $data]);
        }
    ...
    public function authorizeResult($orderId,$data)
    {
        var_dump($data);
        die();
        $response_data = session($orderId);
        if($response_data->getResAuthCode() == '000000'){
