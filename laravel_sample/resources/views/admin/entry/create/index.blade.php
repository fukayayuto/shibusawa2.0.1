...
<div class="row">
    <table class="table">
        ...
        <form action="{{ route('entry_create_confirm_post') }}" method="post" id="form">
            @csrf
            <input type="hidden" id="id" name="id" value="{{ $reserve_data->id }}">
            <input type="hidden" id="reserve_name" name="reserve_name" value="{{ $reserve_data->name }}">
            <input type="hidden" id="start_date" name="start_date" value="{{ $reserve_data->start_date }}">

            <tbody>
                <tr>
                    <td>予約名</td>
                    <td>{{ $reserve_data->name }} </td>
                </tr>
                <tr>
                    <td>開始日</td>
                ...
                <tr>
                    <td>代表者名</td>
                    <td><input type="text" name="name" id="name" class="form-control" value="{{ $data['name'] }}" required></td>
                </tr>

                <tr>
                    <td>代表者名(カナ)</td>
                    <td><input type="text" name="name_kana" id="name_kana" class="form-control" value="{{ $data['name_kana'] }}" required></td>
                </tr>

                <tr>
                    <td>メールアドレス</td>
                    <td><input type="email" name="email" id="email" class="form-control" value="{{ $data['email'] }}" required></td>
                </tr>

                <tr>
                    <td>電話番号</td>
                    <td><input type="phone" name="phone" id="phone" class="form-control" value="{{ $data['phone'] }}" required></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="adult_check" id="adult_check" class="form-control">
                            <option value="0" <?php if ($data['pay_method'] == 0) {
                                echo 'selected';
                            } ?>>いない</option>
                            <option value="1" <?php if ($data['pay_method'] == 1) {
                                echo 'selected';
